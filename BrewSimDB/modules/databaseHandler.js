...
function getStyleByRecipe(name, callback) {
    var sql = 'SELECT beer_recipe.name, beer_style.* FROM beer_style, beer_recipe, style_of_recipe ' +
        'WHERE beer_recipe.name LIKE ? '+
...
function getIBUByRecipe(name, callback) {
    var sql = 'SELECT @gravityOfBoil := SUM((grain.potential_extract * grain_in_recipe.amount * equipment.extract_efficeny)/equipment.batch_size/1000) '+
        ...
        'AND beer_recipe.id = grain_in_recipe.recipe_id AND grain.id = grain_in_recipe.grain_id; '+
        'SELECT beer_recipe.name, FORMAT(SUM(((hops.alpha_acid) * (1.65 * POW(0.00125, (@gravityOfBoil)))*((1.0-POW(2.71828, (-(0.04)*hops_in_recipe.exposure_time))) / 4.15 ) * 74.89)/equipment.batch_size), 1) AS IBU '+
...
function getABVByRecipe(name, callback) {
    ...
    'WHERE beer_recipe.name = ? AND equipment.id = 1 '+
    'AND beer_recipe.id = grain_in_recipe.recipe_id AND grain.id = grain_in_recipe.grain_id; '+
    'SELECT beer_recipe.name, FORMAT(((@OG - (1+((SUM((grain.potential_extract * grain_in_recipe.amount * equipment.extract_efficeny)/equipment.batch_size)-1)*(1-yeast.apparent_attenuation))))*0.129), 1) AS ABV '+
